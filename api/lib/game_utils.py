...
def move_ai_game(author, game, move_intent, stockfish):
    ...
    if gameover_text is not None:
        ...
        complete_game(game)
        return utils.respond(utils.relay_move_db(author, best_move) + '\n' + gameover_text, 202)
...
def move_pvp_game(mover, game, move_intent, stockfish):
    ...
    return utils.respond(f'ur move has been made good job pogO {utils.mention_db_player(mover)}', 202)
...
def get_game(mover, opponent, is_ai):
    ...
    if mover_p is None:
        return GetGameResponse.constructError(
            utils.respond(f'bruh is this your first time {utils.mention_player(mover)}?', 400)
        )
    ...
    if opponent is not None:
        ...
        if opponent_p is None:
            return GetGameResponse.constructError(
                utils.respond(f'Cool you wanna play with {utils.mention_player(opponent)}... but idk who they are.', 400)
            )
    ...
    logger.debug(f"Returning Game: {game} and Mover: {mover_p} and opponent {opponent_p}")
